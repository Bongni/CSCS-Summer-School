...
for n in `seq 10 10`
do
    OMP_NUM_THREADS=1 srun -A summerschool-course2026-cscs -n1 -c12 --hint=nomultithread ./axpy_omp $n > run_omp
    srun -A summerschool-course2026-cscs ./axpy $n > run_cuda
...
do
    srun -A summerschool-course2026-cscs -n1 -c12 --hint=nomultithread ./axpy_omp $n > run_omp
    srun -A summerschool-course2026-cscs ./axpy $n > run_cuda
    time_omp=`grep ^axpy run_omp | awk '{printf("%16.10f", $2)}'`
    time_cuda=`grep ^axpy run_cuda | awk '{printf("%16.10f", $2)}'`
    speedup=`echo $time_omp/$time_cuda | bc -l`
